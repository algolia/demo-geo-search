...
airports.each do |airport|
  airport_info = airport.delete('"').split ','
  records[(airport_info[0]).to_s] = {
...
routes.each do |route|
  route_info = route.delete('"').split ','
  records[(route_info[3]).to_s][:nb_airline_liaisons] += 1 if records[(route_info[3]).to_s]
  records[(route_info[5]).to_s][:nb_airline_liaisons] += 1 if records[(route_info[5]).to_s]
end
...
index.clear
index.set_settings(attributesToIndex: %w(country city name airport_id),
